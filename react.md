...
Webpack.config.js:
There are many ways to configure webpack, but here are three key features:
Entry:  the starting file for webpack. Dependencies will be built from there
...
Module: has many properties, but we’re mostly concerned with loaders for now
