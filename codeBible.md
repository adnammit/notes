...
* **functional requirements**: what the system should do. these are the requirements that are most often used to determine if a system is working correctly. functional requirements are usually expressed as a set of use cases or user stories
* **non-functional requirements**: how well the functional requirements are met. think of the "ilities": scalability, availability, reliability, usability, maintainability and also performance, security, retention, capacity. these can frequently be measured ("how long does it take to load a page?", "how many users can be on the system at once?") and are frequently at odds with one another, i.e. improving performance may result in less readable (maintainable) code. NFRs often heavily influence the architecture of a system: which technologies to use, how to structure the code, etc
